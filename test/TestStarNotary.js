...
it("can Create a Star", async () => {
  ...
  await instance.createStar("Feb Token", tokenId, { from: accounts[0] });
  assert.equal(await instance.tokenIdToStarInfo.call(tokenId), "Feb Token");
});
...
it("lets user1 get the funds after the sale", async () => {
  ...
  await instance.createStar("Feb Token", starId, { from: user1 });
  await instance.putStarUpForSale(starId, starPrice, { from: user1 });
...
it("lets user2 buy a star and decreases its balance in ether", async () => {
  ...
  let starPrice = web3.utils.toWei(".01", "ether");
  let balance = web3.utils.toWei(".05", "ether");
  await instance.createStar("Feb star", starId, { from: user1 });
  await instance.putStarUpForSale(starId, starPrice, { from: user1 });
  ...
  assert.equal(value - 425984, starPrice);
});
...
it("has correct Token Name and Token Symbol", async () => {
  ...
  assert.equal(tokenName, "Feb Token"); // Assert if the starName property was initialized correctly
  let tokenSymbol = await instance.symbol.call();
  assert.equal(tokenSymbol, "FTR"); // Assert if the starName property was initialized correctly
});
...
it("lets 2 users exchange stars", async () => {
  ...
  await instance.createStar("Feb Token - 6!", tokenId1, {
    from: accounts[0],
  });
  await instance.createStar("Feb Token - 7!", tokenId2, {
    from: accounts[1],
  });
...
it("lets a user transfer a star", async () => {
  ...
  let instance = await StarNotary.deployed();
  let tokenId = 8;
  await instance.createStar("Feb Token - 8!", tokenId, {
    from: accounts[0],
  });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  let instance = await StarNotary.deployed();
  let tokenId = 9;
  await instance.createStar("Feb Token - 9!", tokenId, {
    from: accounts[0],
  });
  // 2. Call your method lookUptokenIdToStarInfo
  let starName = await instance.lookUptokenIdToStarInfo(tokenId);
  // 3. Verify if you Star name is the same
  assert.equal(starName, "Feb Token - 9!");
});
